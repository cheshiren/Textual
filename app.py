...
class PhotoScreen(UnfocusedScreen):
	...
	def __init__(self, name = None, id = None, classes = None, labels: list[str] = []):
		super().__init__(name, id, classes)
		self.labels = labels
		...
		with Vertical(id="photo_column"):
			...
			with Grid(id="photo_panel"):
				for i in range(len(self.labels)):
					yield PhotoButton(self.labels[i], id=f"photo{i+1}")
		# yield Footer()
...
class TestApp(App):
	...
	LABELS = [i.label for i in PHOTOS]
	...
	def on_mount(self) -> None:
		...
		self.install_screen(CutScreen(text=Cut.intro_text, next_screen="photo_screen"), name="intro_screen")
		self.install_screen(CutScreen(), name="interlude_screen")
		self.install_screen(CutScreen(), name="outro_screen")
		self.install_screen(PhotoScreen(labels=self.LABELS), name="photo_screen")

		self.push_screen("start_screen")
		self.push_screen("photo_screen")
